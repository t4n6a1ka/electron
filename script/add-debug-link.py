...
def add_debug_link_into_binaries(directory, target_cpu):
  for binary in LINUX_BINARIES:
    binary_path = os.path.join(directory, binary)
    if os.path.isfile(binary_path):
      add_debug_link_into_binary(binary_path, target_cpu)

def add_debug_link_into_binary(binary_path, target_cpu):
  try:
  ...
  cmd = [objcopy, '--add-gnu-debuglink=' + debug_name, binary_path]
  execute(cmd)
...
def main():
  args = parse_args()
  if args.file:
    add_debug_link_into_binary(args.file, args.target_cpu)
  else:
